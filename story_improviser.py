...
def build_judge_prompt(user_request: str, draft_story: str) -> str:
    ...
    """
    return f"""
    You are a careful editor and educator helping polish bedtime stories for children.

    Audience: kids aged 5–10 who are about to sleep. The story will be read aloud by a parent.

    Here is the original story request from the adult:
    "{user_request}"
    ...
    Evaluate this story using the following checklist:
    ...
    If the story already meets all criteria, lightly polish the wording for clarity and flow.
    If it does not, rewrite the story from start to finish so that it satisfies all criteria,
...
def build_revision_prompt(user_request: str, current_story: str, feedback: str) -> str:
    """
    Build a prompt that asks the model to revise the existing story according to the user's feedback.
    """
    return f"""
    You are revising a children's bedtime story based on feedback from the adult reader.
    ...
    Revise the story to address the feedback while keeping:
...
def judge_and_improve_story(user_request: str, draft_story: str) -> str:
    ...
    """
    judge_prompt = build_judge_prompt(user_request, draft_story)
    improved_story = call_model(judge_prompt, max_tokens=1500, temperature=0.4)
...
def revise_story(user_request: str, current_story: str, feedback: str) -> str:
    ...
    """
    revision_prompt = build_revision_prompt(user_request, current_story, feedback)
    revised_story = call_model(revision_prompt, max_tokens=1500, temperature=0.6)
